...
# Dark theme colors
COLORS_DARK = {
    'environmental': '#4ade80',  # Lighter green
    'social': '#4c9aff',         # Lighter blue
    'governance': '#a78bfa',     # Lighter purple
    'total': '#e5e7eb',          # Light gray
...
def get_bg_color():
    ...
    return '#1a1f2e' if CURRENT_THEME == 'dark' else '#ffffff'


def get_text_color():
    """Get text color for current theme"""
    return '#e6e6e6' if CURRENT_THEME == 'dark' else '#1a1a1a'
...
def create_esg_gauge(score: float, title: str, color: str = None) -> go.Figure:
    ...
    if CURRENT_THEME == 'dark':
        steps = [
            ...
            {'range': [40, 60], 'color': '#3f2f1d'},
            {'range': [60, 80], 'color': '#1d3f1f'},
            {'range': [80, 100], 'color': '#0d2f0f'}
        ]
        ...
        go.Bar(
            ...
            marker_color=top_emitters['environmental_score'].apply(
                lambda x: COLORS['positive'] if x >= 60 else (
                    '#FFC107' if x >= 40 else COLORS['negative']
                )
            ),
